...
@app.get("/admin/poll-getupdates")
async def admin_poll_getupdates():
    """
    Engangs-helper: kall Telegram getUpdates for å fange Don's chat_id
    fra siste meldinger til @SesomNodBot. Returnerer alle chat.id +
    chat.type + from-info + text (truncated) for siste 20 updates.

    BRUK: Don sender melding "CAPTURE_DON_CHATID" til @SesomNodBot fra
    sin private DM, deretter Don kaller dette endpointet og leser
    chat.id fra responsen.

    Defensiv: sjekker getWebhookInfo først. Hvis webhook er satt og
    aktiv, returnerer 409 og ber Don bekrefte før vi sletter den.
    """
    token = os.environ.get("TELEGRAM_TOKEN", "")
    if not token:
        return JSONResponse(
            status_code=503,
            content={"error": "TELEGRAM_TOKEN ikke satt"},
        )
    try:
        async with httpx.AsyncClient(timeout=15.0) as client:
            ...
            if webhook_url:
                return JSONResponse(
                    ...
                    content={
                        ...
                        "message": (
                            "getUpdates blokkert fordi webhook er satt. "
                            "Bekreft til Claude at webhook kan slettes "
                            "midlertidig før retry."
                        ),
        ...
        if not updates_data.get("ok"):
            return JSONResponse(
                ...
                content={
                    ...
                    "telegram_response": updates_data,
                },
            )
        ...
        for upd in results:
            ...
            chats.append({
                ...
                "text": text[:80],
                "message_date_unix": msg.get("date"),
            })
        return {
            "status": "ok",
            ...
            "updates": chats,
            "instruction": (
                "Don's chat.id finnes i listen over (chat_type='private'). "
                "Sett som DON_INTERNAL_TELEGRAM_CHAT_ID i Railway env."
            ),
        }
    except Exception as e:
        logger.error("[PollGetUpdates] error: %s", e, exc_info=True)
        return JSONResponse(status_code=500, content={"error": str(e)[:300]})
...
async def admin_verify_pinnacle(days_ahead: int = 3):
    ...
    async with httpx.AsyncClient(timeout=15.0) as client:
        ...
        for league_name, league_id in big5.items():
            ...
            for offset in range(days_ahead + 1):
                target = (today + timedelta(days=offset)).isoformat()
                # API-Football bruker START-year for Big5 crossing-sesonger
                # (verifisert 2026-04-28 via api-football-scan-big5: season=2025
                # for kamper i mai 2026 = 2025/26-sesongen).
                season = today.year if today.month >= 8 else today.year - 1
                try:
                    r = await client.get(
                        f"{base_url}/fixtures",
