...
- Only uses real MiroFish data (pinnacle_clv from picks_v2)
    ...
    try:
        ...
        for row in rows:
            ...
            try:
                ...
                # ── Resolve CLV: picks_v2.pinnacle_clv > pick_receipts.clv_pct ──
                # NEVER invent CLV — only use real data
                clv_pct = None
                clv_source = None

                if row["pinnacle_clv"] is not None:
                    clv_pct = float(row["pinnacle_clv"])
                    clv_source = "picks_v2.pinnacle_clv"
                elif row["receipt_clv"] is not None:
                    clv_pct = float(row["receipt_clv"])
                    clv_source = "pick_receipts.clv_pct"

                # ── Sync CLV from picks_v2 to pick_receipts if available ──
                if row["pinnacle_clv"] is not None and row["receipt_clv"] is None:
                    await db.execute("""
                        UPDATE pick_receipts
                        SET clv_pct = $1, clv_verified = TRUE, clv_source = $2
                        WHERE id = $3
                    """, float(row["pinnacle_clv"]), "atlas_sync", receipt_id)
                    results["clv_synced"] += 1
                    logger.info(
                        f"[ATLAS] CLV synced for {match_name}: "
                        f"{float(row['pinnacle_clv']):.2f}%"
                    )
